...
from models.micro_models import PyramidNetworkCIFAR as PyrmNASNet,AuxiliaryHeadImageNet as AHImageNet
...
parser.add_argument('--check_only', action='store_true', default=False, help='Do not train, only check')
...
def main():
    ...
    if args.net_type == 'micro':
        ...
        net = PyrmNASNet(args.init_channels, num_classes=100, layers=args.layers,
        ...
        net = EvoNetwork(genome, channels, 100, (32, 32), decoder='dense')        
    elif args.net_type=='nano':
        logging.info("==> Building nano search space encoded architectures")
        genotype = eval("genotypes.%s" % args.arch)
        net = AHImageNet(args.init_channels, num_classes=100)
        net.droprate=0.0
    else:
    ...
    if args.check_only==True:
        logging.info("Checking only done...")
        return
