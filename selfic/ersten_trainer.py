...
class ErstenTrainer:
    def __init__(self, epochs=50, batch_size=128, learning_rate=0.025, momentum=0.9, weight_decay=3e-4, cutout_length=16, net_type="ersten", warm=1, resume=False):
        ...
        if net_type=="ersten":
            ...
            self.model = SqueezeNet(class_num=100).to(self.device)
        elif net_type=="mobilenetv2":
            self.model = MobileNetV2(num_classes=100).to(self.device)
        else:
        ...
        self.warmup_scheduler = WarmUpLR(optimizer, self.iter_per_epoch*warm)
